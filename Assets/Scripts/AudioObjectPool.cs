    ...
    void ResetNearbyAudioSources()
    {
        ...
        for(int a = 0; a < activeAudioSources.Count; a++)
        {
            if(activeAudioSources[a] != null)
            {
                ...
                {
                    float dist = Vector3.Distance(activeAudioSources[a].transform.position, fpc.transform.position);
        ...
        Collider[] hitColliders = Physics.OverlapSphere(fpc.transform.position, listeningRadius);
        int i = 0;
        while (i < hitColliders.Length)
        {
                ...
                !audioObjects.Contains(audioObj))
            {
                ...
                float distanceAway = Vector3.Distance(hitColliders[i].transform.position, fpc.transform.position);
